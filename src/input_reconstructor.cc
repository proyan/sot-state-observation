...
namespace sotStateObservation
{
  ...
  void InputReconstructor::computeInert(const dynamicgraph::Matrix & inertia, const dynamicgraph::Matrix & homoWaist, dynamicgraph::Vector& inert, const dynamicgraph::Vector& comVector)
  {
    ...
    waist(0)=homoWaist(0,3);
    waist(1)=homoWaist(1,3);
    waist(2)=homoWaist(2,3);
  ...
  dynamicgraph::Vector& InputReconstructor::computeInput(dynamicgraph::Vector & input, const int& inTime)
  {
    ...
    for(i=0;i<3;++i){
      if(config_[0] & config_[1]) input(u)=comVector(i+3);
      u++;
    }

    for(i=0;i<3;++i){
      if(config_[0] & config_[1] & config_[2]) input(u)=comVector(i+6);
      u++;
    }
    ...
    for(i=0;i<6;++i){
      if(config_[0] & config_[1]) input(u)=dinert(i);
      u++;
    }
    ...
    dangMomentumOut = m*com.cross(comddot);

    for(i=0;i<3;++i){
      if(config_[0] & config_[1]) input(u)=angMomentumOut(i);
      u++;
    }


    for(i=0;i<3;++i){
      if(config_[0] & config_[1] & config_[2]) input(u)=dangMomentumOut(i);
      u++;
    }
    ...
    for(i=0;i<6;++i){
      if(config_[0] & config_[1]) input(u)=imuVector(i+6);
      u++;
    }

    for(i=0;i<3;++i){
      if(config_[0] & config_[1] & config_[2]) input(u)=imuVector(i+12);
      u++;
    }

    for(i=0;i<6*nbContacts;++i){

      input(u)=contactsPosition(i)+bias_[i/6](i%6);
      u++;
    }
